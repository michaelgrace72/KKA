...
function astar(graph, start, goal) {
    let open_set = [{ score: 0, node: start }];
    let came_from = {};
    let g_score = { [start]: 0 };

    while (open_set.length > 0) {
        let { score: current_score, node: current_node } = open_set.shift();

        if (current_node === goal) {
            let path = reconstructPath(came_from, goal);
            return path;
        }

        for (let [neighbor, distance] of Object.entries(graph[current_node])) {
            let tentative_g_score = g_score[current_node] + distance;
            if (!(neighbor in g_score) || tentative_g_score < g_score[neighbor]) {
                g_score[neighbor] = tentative_g_score;
                let f_score = tentative_g_score + heuristic(neighbor, goal);
                open_set.push({ score: f_score, node: neighbor });
...
function reconstructPath(came_from, current_node) {
...
function findRoute(startRute, endRute, startHalte, endHalte) {
    ...
    let goalRute = endRute;
    let goalHalte = endHalte;

    let transitKey = [currentRute, goalRute];
    if (transitKey in transitDict) {
        let transitHalte = transitDict[transitKey];

        let pathToTransit = astar(graph[startRute], startHalte, transitHalte);
        if (pathToTransit) {
            console.log(`Rute tercepat dari ${startHalte} ke ${transitHalte}: ${currentRute} - ${pathToTransit}`);
        } else {
            console.log("Rute tidak tersediaaaa.");
        }

        console.log(`Pindah Rute dari ${currentRute} ke ${goalRute} melalui ${transitHalte}`);
        currentRute = goalRute;
        currentHalte = transitHalte;

        let pathToGoal = astar(graph[currentRute], transitHalte, goalHalte);
        if (pathToGoal) {
            console.log(`Rute tercepat dari ${transitHalte} ke ${goalHalte}: ${currentRute} - ${pathToGoal}`);
        } else {
            console.log("Rute tidak tersedia.");
        }

    } else {
        if (currentRute === "R1") {
            let transit1 = 'Manyar Kerta Adi';
            let nextRute = "R2";
            let pathToTransit = astar(graph[startRute], startHalte, transit1);
            if (pathToTransit) {
                console.log(`Rute tercepat dari ${startHalte} ke ${transit1}: ${currentRute} - ${pathToTransit}`);
                console.log(`Pindah Rute dari ${currentRute} ke ${nextRute} melalui ${transit1}`);
            } else {
                console.log("Rute tidak tersedia.");
            }
            currentRute = nextRute;
            startHalte = transit1;
            findRoute(currentRute, endRute, startHalte, endHalte);
        }

        if (currentRute === "R3") {
            let transit2 = 'Kaliasin';
            let nextRute = "R2";
            let pathToTransit = astar(graph[startRute], startHalte, transit2);
            if (pathToTransit) {
                console.log(`Rute tercepat dari ${startHalte} ke ${transit2}: ${currentRute} - ${pathToTransit}`);
                console.log(`Pindah Rute dari ${currentRute} ke ${nextRute} melalui ${transit2}`);
            } else {
                console.log("Rute tidak tersedia.");
            }
            currentRute = nextRute;
...
let startHalte = prompt("Masukkan halte awal: ");
let endHalte = prompt("Masukkan halte akhir: ");

let startRute, endRute;
for (let [rute, haltes] of Object.entries(graph)) {
...
>>>>>>> d3879bd6f610b3b91c5880ffe933c9985448bf54
findRoute(startRute, endRute, startHalte, endHalte);